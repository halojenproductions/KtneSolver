...
export const colours = [["Red", "red"], ["Blue", "blue"], ["Green", "green"], ["Yellow", "yellow"]];

export function renderSimon(): void {

	var bomb_inputs = getById("SimonAttributes");
	var bomb = [["serial", "vowel, X"], ["strikes", "0,1,2"]]
	...
	for (var ii = 0; ii <= bomb.length - 1; ii++) {
		...
		for (var iii = 0; iii <= options.length - 1; iii++) {
			...
			label.appendChild(document.createTextNode(`${options[iii]} ${bomb[ii][0]}`));
	...
	for (let i = 0; i <= 5; i++) {
		...
		for (let ii = 0; ii <= 3; ii++) {
			...
			input.className = `btn-check simon ${colours[ii][1]}`;
			input.setAttribute("type", "radio");
