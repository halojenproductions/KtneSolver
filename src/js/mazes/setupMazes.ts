...
export function setupMazes(): void {
	Array.from(document.querySelectorAll("#Mazes label.btn")).forEach((element) => {
		element.addEventListener("click", buttonDeselect);
		element.addEventListener("click",(e) => setTimeout(mazes_solve, 0, getById((<HTMLLabelElement>e.target).htmlFor)));
	});
